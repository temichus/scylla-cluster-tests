...
class PythonSCTLogCollector(BaseSCTLogCollector):
    ...
    def _compress_file(self, src_path: str, src_name: str) -> list[str]:
        ...
        """
        LOGGER.info(f"file  = {src_path}")  # pylint: disable=logging-fstring-interpolation
        LOGGER.info(f"getsize = {os.path.getsize(src_path)}")  # pylint: disable=logging-fstring-interpolation
        LOGGER.info(f"limit = {self.too_big_log_size}")  # pylint: disable=logging-fstring-interpolation
        if os.path.getsize(src_path) < self.too_big_log_size:
            LOGGER.info("_compress_file as usual")
            return [super()._compress_file(src_path, src_name)]
        else:
            LOGGER.info("_compress_file via bash")
